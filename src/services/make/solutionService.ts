...
export async function generateSolution(discoveryData: DiscoveryState): Promise<SolutionResponse> {
  ...
  const data = await makeApiRequest<unknown>('solution', payload);

  if (!validateSolutionResponse(data)) {
    throw new ValidationError('Invalid response format from Make.com solution webhook');
  }
